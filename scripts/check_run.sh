...
# Build tinyhttpserver tarball
TARBALL=$(Rscript -e 'cat(devtools::build(".", quiet = TRUE))')
if [ -z "${TARBALL}" ]; then
	echo "Failed to build tarball for tinyhttpserver."
	exit 1
fi
...
if [ -f "./tinyhttpserver.Rcheck/00install.out" ]; then
	cp "./tinyhttpserver.Rcheck/00install.out" "./check-gcc-clang/install-${std}-${compiler}.log"
