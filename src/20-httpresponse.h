...
public:
  ...
  void onWriteComplete(int status) { delete this; }
};
...
void HttpResponse::writeResponse() {
  ...
  if (_statusCode == 101 && _pBody != NULL && _pBody->size() > 0 &&
      _pBody->size() < 256) {
    uv_buf_t buffer = _pBody->getData(_pBody->size());
    if (buffer.len > 0) {
      _responseHeader.reserve(_responseHeader.size() + buffer.len);
    }
    _responseHeader.insert(_responseHeader.end(), buffer.base,
  ...
  uv_buf_t headerBuf =
      uv_buf_init(safe_vec_addr(_responseHeader), _responseHeader.size());
