#ifndef HTTPSERVER_26_HTTP_H
#define HTTPSERVER_26_HTTP_H
...
// using `new`, and puts it inside of the external_pointer. When the
// external_pointer is garbage collected, the shared_ptr is deleted, which
// decrements the refcount.
//
// As long as R has the external_pointer object, the shared_ptr's target won't
// be deleted. Also, when the external_pointer gets GC'd, the shared_ptr will
// get deleted, and if the refcount goes to 0, then the target object will be
// deleted (or, if it has a deleter, that will be called). This means that the
// target object could be deleted from the main thread due to a GC event in R.
inline external_pointer<
...
externalize_shared_ptr(std::shared_ptr<WebSocketConnection> obj) {
  ...
  std::shared_ptr<WebSocketConnection> *obj_copy =
      new std::shared_ptr<WebSocketConnection>(obj);
